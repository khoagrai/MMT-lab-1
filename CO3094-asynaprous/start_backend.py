"""Entry point for deploying a backend server process."""

import argparse
import os

from daemon import create_backend

PORT = 9000


if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="Backend", epilog="Backend daemon")
    parser.add_argument("--server-ip", type=str, default="0.0.0.0")
    parser.add_argument("--server-port", type=int, default=PORT)
    parser.add_argument(
        "--async-mode",
        choices=["threading", "callback", "coroutine"],
        default=None,
        help="Concurrency mode. Overrides ASYNC_MODE environment variable.",
    )
    args = parser.parse_args()

    if args.async_mode:
        os.environ["ASYNC_MODE"] = args.async_mode
    create_backend(args.server_ip, args.server_port)
